...
date: "HfWU Zukunftsökonomie | SoSe 2024"
...
- Unterlagen online, s. QR-Code

- Prüfungsleistung: K 60
...
- Für **langfristige Lösungen** ist Effizienz von großer Bedeutung.
...
```{r echo=F, message=F, warning=F, cache=T, cache.comments=FALSE}
...
[s. Veranstaltung Science Lab](https://janvoss.github.io/Science_Lab/wissenschaftstheorie.html)

## Rolle der Wissenschaft in der Gesellschaft

[s. Veranstaltung Science Lab](https://janvoss.github.io/Science_Lab/wissenschaftstheorie.html)

# Soziale Phänomene und „Gleichgewichte"

[Feedback, aber schöner? ]::
...
mydata %>%
  ...
  labs(title="Entwicklung des BIPs in Deutschland",
       x="Jahr",
       y="BIP",
       caption="Eigene Darstellung, Daten: Eurostat Tabelle nama_10_gdp")
...
```  
Die durchschnittliche Wachstumsrate des Bruttoinlandsprodukts für Deutschland im Beobachtungszeitrum (blaue Linie) beträgt `r round(g,2)`%.

```{r echo=F, warning=F, message=F, cache=F, eval=T}  
mydata %>%
  filter(geo=="Germany") %>%
  ggplot(aes(TIME_PERIOD, growth))+
  geom_line(#color="red"
    )+
  geom_point()+
  #geom_smooth(method = 'lm'#, formula = y ~ x + I(x^2))
  ...
  labs(title="Veränderungsraten des BIPs in Deutschland", 
       x="Jahr",
       y="Veränderungsrate",
       caption="Eigene Darstellung, Daten: Eurostat Tabelle nama_10_gdp")
...
model <- lm(log(values) ~ TIME_PERIOD, data= mydata %>%                             filter(geo=="Germany"))
...
Unerschiedliche Suchrichtungen für Erklärungen (in sehr enger Anlehnung an @blum2016grundlagen)

![Suchrichtungen für Erklärungen](./TiKz/Deduktion.svg)
...
Ziele: 
  ...
  - Möglichst gut und viel konsumieren (Bedürfnisse stillen, Nutzen maximieren)  
  - Wenig zahlen  
...
Restriktionen:  
  ...
  - Zeit
  - Preise  
  - Mangelndes Wissen/Unsichere Zukunft  
  - $\dots$
...
Ziele: 
  ...
  - Gewinn maximieren  
  - Kundenbindung/ Gutes Image  
  - Überschaubare Risiken/ Sicherheit  
  - ggf. Kostenreduktion  
  - Hohe Marktanteile 
  - (Arbeitskräfte)  
  - Innovation 
...
Restriktionen:  
  ...
  - Fachkräftemangel  
  - Begrenzte Produktionsmittel  
  - ggf. schlechtes Image  
  - Standort/ Region  
  - Rechtliche Restriktionen  
  - Finanzmittel  
  - Begrenzte Absatzkanäle  
...
Ziele:
  ...
  - Zustimmung/Wiederwahlinteresse 
  - Macht
  - Geld/Steuereinnahmen  
  - Wohlstand, Zufriedenheit, Lebensqualität der Bürger
...
Restriktionen:
  ...
  - Bevölkerung  
  - Parteienkonkurrenz  
  - Globale Politik/internationale Abkommen  
  - Information/Image
  - $\dots$
...
Restriktionen:  
  ...
  - Konsumverhalten der Konsumenten

**Supranationale Organisationen**

**NGOs/Aktivisten/Stiftungen**

Ziele:  

  - Handeln der Politik beeinflussen/erzwingen  
  - Möglichst viele Anhänger, Macht, Ruhm, Reichweite 
  - Gemeinwohlziele erreichen  
  - $\dots$
...
Restriktionen:
  ...
  - Rechtslage  
  - Bevölkerung  
  - Mangelnde Handlungs- und Umsetzungsfähigkeit  
  - $\dots$
...
Ziele: 
  ...
  - Angenehmes Arbeitsumfeld/Vorgesetzte/Wertschätzung, Unterstützung im Konflikt (Gewerkschaften, Betriebsrat)  
  - Selbstverwirklichung/Karriere  
...
Restriktionen:
  ...
  - Standort  
  - Märktewandel  
  - Lebensumstände/familiäre Verantwortung 
...
# Sammlung WiSe 2023/24

- **Haushalt/Familie**

  - Zielfunktion?  
    
      - Langfristige Optimierung/ Sparen für die Zukunft
...
- **Der Staat**
  ...
  - Restriktionen?
    ...
    - Relevanz auf internationaler Bühne
    
    - Ressourcen
    
    - Rechtliche Rahmenbedingunge
    
    - Öffentliche Meinung (Opposition, Presse)
    
    - Bevölkerung (Wahlen, Abstimmungen)
...
```{r Patente, echo=F, warning=F, message=F, cache=T, eval=F}
...
```{r Patente2, echo=F, warning=F, message=F, cache=T, eval=F}
...
```{r migration, echo=F, message=F, warning=F, cache=T, eval=F}
...
ggplotly(p) %>% 
  config(displaylogo = F) %>% 
  layout(margin = list(l = 50, r = 50, b = 100, t = 50),
         annotations = list(x = 1, y = -0.3, text =  paste('Abbildung: Jan S. Voßwinkel; Daten: World Bank; Datenabruf:', format(Sys.Date(), format="%d.%m.%Y"), sep = " "),
                            xref='paper', yref='paper', showarrow = F, 
                            xanchor='right', yanchor='auto', xshift=0, yshift=0,
                            font = list(size = 11)))
```
...
# Anwendung: Das aktuelle Frühjahrsgutachten des Sachverständigenrates zur Begutachtgung der gesamtwirtschaftlichen Entwicklung {-}

## Der Sachverständigenrat {-}

[Zum Sachverständigenrat](https://www.sachverstaendigenrat-wirtschaft.de/)

## Das aktuelle Frühjahrsgutachten {-}

[Das aktuelle Frühjahrsgutachten](https://www.sachverstaendigenrat-wirtschaft.de/fruehjahrsgutachten-2024.html?returnUrl=%2Findex.html&cHash=b112beed71f5312cb584097f9468c4ae)


```{r,  echo=FALSE, warning=FALSE, out.width="30%"}
library(qrcode)

qr <- qr_code('https://www.sachverstaendigenrat-wirtschaft.de/fruehjahrsgutachten-2024.html?returnUrl=%2Findex.html&cHash=b112beed71f5312cb584097f9468c4ae')
plot(qr)

```

1. Wir verschaffen uns zunächst einen Überblick mit Hilfe der Pressemitteilung

1. Schauen wir genauer in des Kapitel zum Güterverkehr

[Kapitel 2 zum Güterverkehr](https://www.sachverstaendigenrat-wirtschaft.de/fileadmin/dateiablage/gutachten/fg2024/FG2024_Kapitel_2.pdf)

```{r,  echo=FALSE, warning=FALSE, out.width="30%"}
library(qrcode)

qr <- qr_code('https://www.sachverstaendigenrat-wirtschaft.de/fileadmin/dateiablage/gutachten/fg2024/FG2024_Kapitel_2.pdf')
...
```
 ...
 a) Welche Effizienzprobleme werden angesprochen?
    
 a) Welche Lösungsstrategien werden diskutiert?
    
  a) Wie bewerten Sie die Darstellung?
...
#  annotate("text", x=1, y=28, label="KR", color="grey30")+
  ...
  geom_ribbon(aes(x = z_wfl, ymin = demand(z_wfl), ymax = demand_ext(z_wfl),
                                 fill = "Soziale Zusatznutzen"), alpha = 0.55)+
 # annotate("text", x=q+1, y=p_t, label="Wohlfahrtsverlust",color = "grey30")+
  scale_color_manual(values=c("green", "orange", "red"))+
  scale_fill_manual(values=c("red", "green", "orange"))+
  scale_x_continuous(expand = c(0, 0), 
...
# Institutionen- und Konstituonenökonomik
