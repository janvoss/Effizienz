...
**Definition**: Das **Bruttoinlandsprodukt (BIP)** ist der **Marktwert** aller für den **Endverbrauch** bestimmten **Waren und Dienstleistungen**, die **in einem Land** **in einem bestimmten Zeitabschnitt** hergestellt werden.
...
```{r echo=T, warning=F, message=F, output=F}
library(dplyr)
library(ggplot2)
...
p <- df %>% filter(entity %in% selected_countries) %>% 
  ggplot(aes(year,ny_gdp_pcap_pp_kd))+
  geom_point()+
  geom_line()+
...
growth <- df %>% filter(entity %in% selected_countries) %>% 
  group_by(entity) %>% 
  do(tidy(lm(log(ny_gdp_pcap_pp_kd) ~ year,.))) %>% 
  filter(term=="year") %>% 
...
kable(growth, digits=2, format.args = list(decimal.mark = ','), col.names=c("Staat", "Wachstumsrate (%)"),
      caption="geschätzte Durchschnittliche jährliche Wachstumsrate (%) 1990-2024, Daten: Ourworldindata.org")
...
Gespart wird, indem produziert, aber nicht konsumiert wird. 

Geschlossene Volkswirtschaft: 

$$S=Y-C-G$$

Mit $$Y=C+G+I \iff I=Y-C-G$$
...
$$Y=C+I+G+NX \iff Y-C-G=I+NX \iff I=Y-C-G-NX$$

$$\iff I=S-NX$$

Heimische Investitionen und Nettoexporte hängen also zusammen. 

- Nominales vs. reales BIP

  - **Nominales BIP**: Messung zu laufenden Preisen
  - Das nominale BIP verändert sich also sowohl durch Mengen- als auch durch Preisänderungen
  
  - **Reales BIP**: Messung zu konstanten Preisen eines Basisjahres
  - Das reale BIP verändert sich nur durch Mengenänderungen
