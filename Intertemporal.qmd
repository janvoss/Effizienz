...
Wenn eine Ressource in kontinuierlicher Zeit mit der Wachstumsrate rg$ wächst, und sie ursprünglich $X_0$ beträgt, dann beträgt sie nach $T$ Perioden $X_T=X_0 \times  e^{r\times T}$ Einheiten.
...
Aus der Ableitung der Nutzenfunktion $u'(x_t)= \frac{1}{x_t}$ folgt der optimale Konsumpfad (**Euler-Gleichung**): $$\boxed{x_{t+1}=x_t \times e^{r-i}}$$.

Da dies für alle Perioden gilt, ist $$x_t= x_0\ e^{(r-i)t}$$


Wie bestimmt sich aber $x_0$?
...
Aus der Euler-Gleichung folgt bei der Nutzenfunktion $u(x_t)=ln(x_t)$, dass der Konsum mit der Rate $r-i$ wachsen soll.

Von den Erträgen auf die Resourcenausstattung $r\times X_0$ muss also der Anteil $r-i$ zurückgelegt werden, damit die Erträge in der nächsten Perioden entsprechend höher sein können. Der Rest kann konsumiert werden.

Also: $x_0=\underbrace{X_0\times r }_{\text{Erträge auf den Bestand}} \underbrace{- X_0 \times (r-i)}_{\text{Rücklage für späteren Konsum}}=X_0(r-r+i)=iX_0$
...
Notation: Ein Punkt über einer Variable bezeichnet ihre Veränderung in der Zeit.
